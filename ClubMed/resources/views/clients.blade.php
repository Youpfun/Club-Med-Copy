<!DOCTYPE html> <html lang="fr"> <head> <meta charset="UTF-8"> 
    ...
    <h1>Annuaire des Clients</h1> <table> <thead> <tr> <th>N° 
            ...
            </tr> </thead> <tbody> @foreach($clients as $client) <tr> 
                    <td>{{ $client->numclient }}</td>
                    
                    <td> {{ $client->nomclient }} {{ 
                        $client->prenomclient }} <br> <small>({{ 
                        $client->genreclient }})</small>
                    </td> <td>{{ $client->login }}</td> <td>{{ 
                    $client->emailclient }}</td> <td>{{ 
                    $client->telephone }}</td>
                    
                    <td> {{ $client->numrue }} {{ $client->nomrue }}<br> 
                        {{ $client->codepostal }} <strong>{{ 
                        $client->ville }}</strong>
                    </td> <td>{{ $client->datenaissance }}</td> </tr> 
            @endforeach
        </tbody> </table> </body>
</html>
