        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-2xl shadow-md p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="font-medium text-gray-900">{{ $prospection->nom_resort }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500">Email</p>
                            <p class="font-medium text-gray-900">{{ $prospection->email_resort }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500">Pays</p>
                            <p class="font-medium text-gray-900">{{ $prospection->pays ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500">Ville</p>
                            <p class="font-medium text-gray-900">{{ $prospection->ville ?? '-' }}</p>
                        </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-md p-6">
                    ...
                    <div class="space-y-3">
                        {{-- Renvoyer l'email --}}
                        <form action="{{ route('marketing.prospection.resend', $prospection->numprospection) }}" method="POST">
                            @csrf
                        ...
                        {{-- Supprimer --}}
                        <form action="{{ route('marketing.prospection.destroy', $prospection->numprospection) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette prospection ?')">
                            @csrf
                            @method('DELETE')
                ...
                <div class="bg-white rounded-2xl shadow-md p-6">
                    ...
                    <form action="{{ route('marketing.prospection.update-statut', $prospection->numprospection) }}" method="POST" class="space-y-4">
                        @csrf
                        @method('PUT')

                        <div>
                            <label for="statut" class="block text-sm font-medium text-gray-700 mb-1">Statut</label>
                            <select name="statut" id="statut" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-purple-500 focus:ring focus:ring-purple-200">
                                @foreach(\App\Models\ProspectionResort::getStatuts() as $key => $label)
                        ...
                        <div>
                            ...
                            <textarea name="reponse" id="reponse" rows="4" 
                                ...
                                placeholder="Notez ici la réponse du resort ou vos remarques...">{{ $prospection->reponse }}</textarea>
                        </div>

                        <button type="submit" class="w-full px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-bold transition">
                            Mettre à jour
                        </button>
                    </form>
                </div>
