    ...
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            @php
                ...
                $envoyees = \App\Models\ProspectionPartenaire::where('statut', 'envoyee')->count();
                $repondues = \App\Models\ProspectionPartenaire::where('statut', 'repondue')->count();
                $enCours = \App\Models\ProspectionPartenaire::where('statut', 'en_cours')->count();
            @endphp
            <div class="bg-white rounded-xl p-4 shadow-sm border-l-4 border-emerald-500">
                <p class="text-2xl font-bold text-emerald-600">{{ $totalProspections }}</p>
        ...
        <div class="bg-white rounded-2xl shadow-md overflow-hidden">
            @if($prospections->isEmpty())
                <div class="p-12 text-center">
                    ...
                    <h3 class="text-xl font-bold text-gray-700 mb-2">Aucune prospection partenaire</h3>
                    <p class="text-gray-500 mb-6">Commencez par contacter un partenaire potentiel (ESF, spa, etc.)</p>
                    <a href="{{ route('marketing.prospection-partenaire.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-bold transition">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
            ...
            @else
                <table class="w-full">
                    <thead class="bg-slate-100">
                        <tr>
                            <th class="px-6 py-4 text-left text-sm font-bold text-gray-700">Partenaire</th>
                            <th class="px-6 py-4 text-left text-sm font-bold text-gray-700">Type d'activité</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach($prospections as $prospection)
                            <tr class="hover:bg-slate-50 transition">
                                <td class="px-6 py-4">
                                    <div class="font-bold text-gray-900">{{ $prospection->nom_partenaire }}</div>
                                    <div class="text-sm text-gray-500">{{ $prospection->email_partenaire }}</div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a href="{{ route('marketing.prospection-partenaire.show', $prospection->numprospection) }}" class="inline-flex items-center px-3 py-1 bg-blue-100 hover:bg-blue-200 text-blue-700 rounded-lg text-sm font-medium transition">
                                        👁️ Voir
                                    </a>
                                </td>
                            </tr>
